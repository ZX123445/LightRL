...
class GhostModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=1, ratio=2):
        super().__init__()
        intrinsic = out_channels // ratio
        self.primary_conv = nn.Sequential(
            nn.Conv2d(in_channels, intrinsic, kernel_size, padding=kernel_size // 2),
            nn.ReLU(inplace=True)
        )
        self.cheap_conv = nn.Conv2d(intrinsic, out_channels - intrinsic,
                                    kernel_size, padding=kernel_size // 2, groups=intrinsic)
...
class LightDecomNet(nn.Module):
    def __init__(self, channel=64, kernel_size=3, num_exposures=5):
        ...
        # 轻量化初始卷积
        self.net1_conv0 = nn.Sequential(
            DepthwiseSeparableConv(input_channels, channel // 2, kernel_size * 3, padding=4),
            ChannelShuffle(groups=4),
            nn.ReLU(),
            DepthwiseSeparableConv(channel // 2, channel, kernel_size, padding=1)
        )

        # 共享权重的卷积层
        self.shared_conv = DepthwiseSeparableConv(channel, channel, kernel_size, padding=1)
        self.shared_act = nn.ReLU()

        # 轻量化主干网络 (使用Ghost模块)
        self.net1_convs = nn.Sequential(
            GhostModule(channel, channel),
            nn.ReLU(),
            self.shared_conv,  # 共享权重
            self.shared_act,  # 共享激活
            GhostModule(channel, channel),
            nn.ReLU(),
            self.shared_conv,  # 共享权重
            self.shared_act,  # 共享激活
            DepthwiseSeparableConv(channel, channel, kernel_size, padding=1),
            nn.ReLU()
        )

        # 重构层
        self.net1_recon = DepthwiseSeparableConv(channel, 4, kernel_size, padding=1)
...
class LightTeacherRelightNet(nn.Module):
    def __init__(self, channel=128, kernel_size=3):
        super().__init__()
        self.relu = nn.ReLU()

        # 轻量化输入层
        self.net2_conv0_1 = DepthwiseSeparableConv(4, channel, kernel_size, padding=1)

        # 下采样路径 (使用深度可分离卷积)
        self.net2_conv1_1 = DepthwiseSeparableConv(channel, channel, kernel_size, stride=2, padding=1)
        self.net2_conv1_2 = DepthwiseSeparableConv(channel, channel, kernel_size, stride=2, padding=1)
        self.net2_conv1_3 = DepthwiseSeparableConv(channel, channel, kernel_size, stride=2, padding=1)
        self.net2_conv1_4 = DepthwiseSeparableConv(channel, channel, kernel_size, stride=2, padding=1)

        # 上采样路径 (使用Ghost模块)
        self.net2_deconv1_1 = GhostModule(channel * 2, channel)
        self.net2_deconv1_2 = GhostModule(channel * 2, channel)
        self.net2_deconv1_3 = GhostModule(channel * 2, channel)
        self.net2_deconv1_4 = GhostModule(channel * 2, channel)

        # 特征融合与输出
        self.net2_fusion = GhostModule(channel * 4, channel)
        self.net2_output = DepthwiseSeparableConv(channel, 1, kernel_size=3, padding=0)
...
class LightRelightNet(nn.Module):
    def __init__(self, channel=64, kernel_size=3, use_distill=False):
        ...
        # 下采样路径 (使用Ghost模块)
        self.net2_conv1_1 = GhostModule(channel, channel)
        self.net2_conv1_2 = GhostModule(channel, channel)
        self.net2_conv1_3 = GhostModule(channel, channel)

        # 上采样路径 (使用深度可分离卷积)
        self.net2_deconv1_1 = DepthwiseSeparableConv(channel * 2, channel, kernel_size, padding=1)
        self.net2_deconv1_2 = DepthwiseSeparableConv(channel * 2, channel, kernel_size, padding=1)
        self.net2_deconv1_3 = DepthwiseSeparableConv(channel * 2, channel, kernel_size, padding=1)

        # 特征融合与输出
        self.net2_fusion = GhostModule(channel * 3, channel)
        self.net2_output = DepthwiseSeparableConv(channel, 1, kernel_size=3, padding=0)

        # 知识蒸馏适配器
        if self.use_distill:
            self.feature_adapters = nn.ModuleDict({
                'out1': DepthwiseSeparableConv(channel, 128, 1),
                'out2': DepthwiseSeparableConv(channel, 128, 1),
                'out3': DepthwiseSeparableConv(channel, 128, 1),
                'feats_fus': DepthwiseSeparableConv(channel, 128, 1)
            })
...
class LightRetinexNet(nn.Module):
    def __init__(self, num_exposures=5, use_distill=False):
        ...
        # 轻量化分解网络
        self.DecomNet = LightDecomNet(num_exposures=num_exposures)

        # 轻量化重光照网络
        self.RelightNet = LightRelightNet(use_distill=use_distill)

        if use_distill:
            # 轻量化教师网络
            self.TeacherRelightNet = LightTeacherRelightNet()
    ...
    def calculate_metrics(self, enhanced, gt):
        """计算PSNR和SSIM指标"""
        psnr = self.calculate_psnr(enhanced, gt)
        ssim_val = self.calculate_ssim(enhanced, gt)
    ...
    def load(self, ckpt_dir):
        ...
        try:
            ...
            if self.train_phase == 'Decom':
                # 检查输入通道数是否匹配
                if 'net1_conv0.0.depthwise.weight' in ckpt:
                    saved_weight = ckpt['net1_conv0.0.depthwise.weight']
                    current_weight = self.DecomNet.net1_conv0[0].depthwise.weight
